...
export default function ShipmentPortList() {
  ...
  const fetchPorts = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/en/api/v1/shipment/port/list/?page=${page}`);
  ...
  const handleViewDetails = (portId) => {
    navigate(`/shipment/ports/${portId}`);
  };

  const handleCreate = () => {
    navigate('/shipment/ports/create');
  };
        ...
        <div className="flex-1 p-5">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Shipment Ports</h1>
              <button
                    ...
                    <table className="min-w-full divide-y divide-gray-700">
                      ...
                      <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {ports.map((port) => {
                          ...
                          return (
                            <tr key={port.id} className="hover:bg-gray-700 transition-colors">
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{port.id}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{port.title}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{port.country}</td>
