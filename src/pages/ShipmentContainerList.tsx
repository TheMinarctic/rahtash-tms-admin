...
export default function ShipmentContainerList() {
  ...
  const fetchContainers = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/en/api/v1/shipment/container/list/?page=${page}`);
  ...
  const handlePageChange = (newPage) => {
    fetchContainers(newPage);
  };

  const handleViewDetails = (containerId) => {
    navigate(`/shipment/containers/${containerId}`);
  };

  const handleCreate = () => {
    navigate('/shipment/containers/create');
  };
        ...
        <div className="flex-1 p-5">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Shipment Containers</h1>
              <button
                    ...
                    <table className="min-w-full divide-y divide-gray-700">
                      <thead className="bg-gray-700">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Track Number</th>
                      ...
                      <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {containers.map((container) => {
                          ...
                          return (
                            <tr key={container.id} className="hover:bg-gray-700 transition-colors">
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{container.id}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{container.track_number}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{container.size}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                {getContainerType(container.type)}
                              </td>
