...
export default function DriverCategoryDetail() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/en/api/v1/driver/category/detail/${category_id}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.patch(`/en/api/v1/driver/category/update/${category_id}/`, formData);
      const response = await api.get(`/en/api/v1/driver/category/detail/${category_id}/`);
      setCategory(response.body.data);
      setEditMode(false);
      setLoading(false);
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        setLoading(true);
        await api.delete(`/en/api/v1/driver/category/delete/${category_id}/`);
        navigate('/driver/categories');
      } catch (err) {
        setError(err.message);
        setLoading(false);
      }
        ...
        <div className="flex-1 p-5 mt-[5%]">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Driver Category Details</h1>
              <button
                onClick={() => navigate('/driver/categories')}
