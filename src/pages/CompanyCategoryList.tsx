...
export default function CompanyCategoryList() {
  ...
  const fetchCategories = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/en/api/v1/company/category/list/?page=${page}`);
  ...
  const handleViewDetails = (categoryId) => {
    navigate(`/company/categories/${categoryId}`);
  };

  const handleCreate = () => {
    navigate('/company/categories/create');
  };
        ...
        <div className="flex-1 p-5">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Company Categories</h1>
              <button
