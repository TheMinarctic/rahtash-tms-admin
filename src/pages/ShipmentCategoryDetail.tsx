...
export default function ShipmentCategoryDetail() {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/en/api/v1/shipment/category/detail/${category_id}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.patch(`/en/api/v1/shipment/category/update/${category_id}/`, formData);
      const response = await api.get(`/en/api/v1/shipment/category/detail/${category_id}/`);
      setCategory(response.body.data);
      setEditMode(false);
      setLoading(false);
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        setLoading(true);
        await api.delete(`/en/api/v1/shipment/category/delete/${category_id}/`);
        navigate('/shipment/categories');
      } catch (err) {
        setError(err.message);
        setLoading(false);
      }
