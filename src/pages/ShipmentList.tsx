...
export default function ShipmentList() {
  ...
  const fetchShipments = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/en/api/v1/shipment/list/?page=${page}`);
                    ...
                    <table className="min-w-full divide-y divide-gray-700">
                      <thead className="bg-gray-700">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">B/L Number</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Carrier</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Loading Port</th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Discharge Port</th>
                      ...
                      <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {shipments.map((shipment) => {
                          ...
                          return (
                            <tr key={shipment.id} className="hover:bg-gray-700 transition-colors">
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{shipment.id}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">
                                {shipment.bill_of_lading_number_id || 'N/A'}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                {shipment.carrier_company?.name || 'N/A'}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
