...
export default function ShipmentStepDetail() {
  ...
  useEffect(() => {
    const fetchStep = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/en/api/v1/shipment/step/detail/${step_id}/`);
        setStep(response.body.data);
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.patch(`/en/api/v1/shipment/step/update/${step_id}/`, formData);
      const response = await api.get(`/en/api/v1/shipment/step/detail/${step_id}/`);
      setStep(response.body.data);
      setEditMode(false);
      setLoading(false);
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this step?")) {
      try {
        setLoading(true);
        await api.delete(`/en/api/v1/shipment/step/delete/${step_id}/`);
        navigate("/shipment/steps");
      } catch (err) {
        setError(err.message);
        setLoading(false);
      }
        ...
        <div className="flex-1 p-5 mt-[5%]">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Step Details</h1>
              <button
                onClick={() => navigate("/shipment/steps")}
                ...
                <>
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-white">{step.order}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-white">{new Date(step.created_at).toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Updated At</p>
                      <p className="text-white">{new Date(step.updated_at).toLocaleString()}</p>
                    </div>
                  </div>
