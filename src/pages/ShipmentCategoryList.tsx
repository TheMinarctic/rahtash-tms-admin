...
export default function ShipmentCategoryList() {
  ...
  const fetchCategories = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/en/api/v1/shipment/category/list/?page=${page}`);
  ...
  const handleViewDetails = (categoryId) => {
    navigate(`/shipment/categories/${categoryId}`);
  };

  const handleCreate = () => {
    navigate('/shipment/categories/create');
  };
        ...
        <div className="flex-1 p-5">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Shipment Categories</h1>
              <button
                        ...
                        {categories.map((category) => {
                          ...
                          return (
                            <tr key={category.id} className="hover:bg-gray-700 transition-colors">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{category.title}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{category.order}</td>
