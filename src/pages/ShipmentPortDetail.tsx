...
export default function ShipmentPortDetail() {
  ...
  useEffect(() => {
    const fetchPort = async () => {
      try {
        setLoading(true);
        const response = await api.get(`/en/api/v1/shipment/port/detail/${port_id}/`);
        setPort(response.body.data);
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await api.patch(`/en/api/v1/shipment/port/update/${port_id}/`, formData);
      const response = await api.get(`/en/api/v1/shipment/port/detail/${port_id}/`);
      setPort(response.body.data);
      setEditMode(false);
      setLoading(false);
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this port?')) {
      try {
        setLoading(true);
        await api.delete(`/en/api/v1/shipment/port/delete/${port_id}/`);
        navigate('/shipment/ports');
      } catch (err) {
        setError(err.message);
        setLoading(false);
      }
  ...
  const statusBadge = getStatusBadge(port.status);
        ...
        <div className="flex-1 p-5">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Port Details</h1>
              <button
                ...
                <>
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-white">{port.id}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Title</p>
                      <p className="text-white">{port.title}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Country ID</p>
                      <p className="text-white">{port.country}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-white">{new Date(port.created_at).toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-400">Updated At</p>
                      <p className="text-white">{new Date(port.updated_at).toLocaleString()}</p>
                    </div>
                  </div>
