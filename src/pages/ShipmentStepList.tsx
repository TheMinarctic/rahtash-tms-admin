...
export default function ShipmentStepList() {
  ...
  const fetchSteps = async (page = 1) => {
    try {
      setLoading(true);
      const response = await api.get(`/en/api/v1/shipment/step/list/?page=${page}`);
      setSteps(response.body.data);
      setPagination({
  ...
  const handleViewDetails = (stepId) => {
    navigate(`/shipment/steps/${stepId}`);
  };

  const handleCreate = () => {
    navigate('/shipment/steps/create');
  };
        ...
        <div className="flex-1 p-5">
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-3xl font-bold text-white">Shipment Steps</h1>
              <button
                    ...
                    <table className="min-w-full divide-y divide-gray-700">
                      ...
                      <tbody className="bg-gray-800 divide-y divide-gray-700">
                        {steps.map((step) => {
                          ...
                          return (
                            <tr key={step.id} className="hover:bg-gray-700 transition-colors">
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{step.id}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{step.title}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{step.order}</td>
